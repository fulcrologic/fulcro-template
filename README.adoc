...
# 1. Start shadow-cljs server and Clojure REPL:
npm run start
# 2. Start the backend server (typing into this terminal where you run `npm run start`):
#    (Note: for proper development you will want to start it in a nREPL-based REPL, see
#     the detailed instructions for "The API Server" below.)
user=> (start)
# after modifications of the backend code run: `(restart)`
# 3.: Visit the shadow-cljs compile server UI at http://localhost:9630
